...
<table border="0" cellspacing="0" style="padding:0px; margin-top:5px; width:98%" id="CommonTable">
    <tr >
        ...
        <td width="17%" style="height:40px; vertical-align:middle;">
            ...
            <form name="form2" action="AdvisorTree.php" method="get" style="margin-top:10PX;" >
                <select name="advisor_sponsor" id="advisor_sponsor" style="font-size:11px;" onchange="form2.submit()" >
                    ...
                    <?php
                    $q = @mysqli_query($_SESSION['CONN'], "select advisor_id, advisor_code, advisor_name, advisor_level_id from tbl_advisor order by advisor_code ");
                    while ($SPONSOR_ROWS = @mysqli_fetch_array($q)) {
                        ?>
        ...
        <td width="22%" style="vertical-align:middle">
            <form name="form1" action="AdvisorTree.php" method="get" style="margin-top:10PX;" >
                NAME&nbsp;
                <select name="advisor_sponsor" id="advisor_sponsor" style="font-size:11px; " onchange="form1.submit()" >
                    ...
                    <?php
                    $q = @mysqli_query($_SESSION['CONN'], "select advisor_id, advisor_code, advisor_name, advisor_level_id from tbl_advisor order by advisor_name");
                    while ($SPONSOR_ROWS = @mysqli_fetch_array($q)) {
                        ?>
                        <option value="<?php echo $SPONSOR_ROWS['advisor_id']; ?>" <?php if ($_GET['advisor_sponsor'] == $SPONSOR_ROWS['advisor_id']) { ?> selected="selected"<?php } ?>>
        ...
        <td width="13%" style="line-height:13px;">
            <font color="#FF0000">X : Inactive</font><br />
            <font color="#00aa33">&#x2713; : Active</font>
        </td>

        <td width="39%">SHOWING TEAM OF : <blink><font color="#0033FF">
        <?php
        if ($_GET['advisor_sponsor']) {
            echo $DBOBJ->ConvertToText("tbl_advisor", "advisor_id", "advisor_name", $_GET['advisor_sponsor']);
        } else {
            echo $DBOBJ->ConvertToText("tbl_advisor", "advisor_id", "advisor_name", 1);
        }
        ?>
        </font></blink></td>
    ...
    if ($path > 0) {
        ...
        while ($rows = @mysqli_fetch_array($s)) {
            ...
            //printSubDir($file, $path, $queue);
                ...
                $ADV_sql = @mysqli_query($_SESSION['CONN'], "select  advisor_id, advisor_code, advisor_name, advisor_level_id,advisor_status from tbl_advisor where advisor_id='" . $rows['advisor_id'] . "' order by advisor_name");
                $rows = @mysqli_fetch_array($ADV_sql);

                $status = " <font color='#ff0000' title='Inactive'>(X)</font>";
                if ($rows['advisor_status'] == "1") {
                    $status = " <font color='#00aa33' title='Active'>(&#x2713;)</font>";
                }

                if ($rows['advisor_name'] != NULL)
                    $queue[] = $rows['advisor_name'] . " / <font color=\"GREEN\"> " . $rows['advisor_code'] . " </font> / <font color=\"#006697\"> " . $DBOBJ->ConvertToText("tbl_setting_advisor_level", "level_id", "level_name", $rows['advisor_level_id']) . "</FONT>" . $status;
            }
        }
...
function printFile($file) {
    ...
    echo '<li style="color:#AA2200; font-weight:bolder;background-color:#FFFFFF; ">' . $file . '</li>';
}

function printSubDir($dir) {
    $DBOBJ = new DataBase();
    $DBOBJ->ConnectDatabase();
    $ADV_sql = @mysqli_query($_SESSION['CONN'], "select  advisor_id, advisor_code, advisor_name, advisor_level_id, advisor_status from tbl_advisor where advisor_id='$dir' order by advisor_name");
    $rows = @mysqli_fetch_array($ADV_sql);
    ...
    $c = $rows['advisor_name'] . " / <font color=\"GREEN\">" . $rows['advisor_code'] . "</font> / <font color=\"#006697\">" . $DBOBJ->ConvertToText("tbl_setting_advisor_level", "level_id", "level_name", $rows['advisor_level_id']) . "</FONT>" . $status;
...
function fprintSubDir($dir) {
    ...
    $c = $rows['advisor_name'] . " /<font color=\"GREEN\"> " . $rows['advisor_code'] . " </font>/ " . $DBOBJ->ConvertToText("tbl_setting_advisor_level", "level_id", "level_name", $rows['advisor_level_id']) . $status;
    ?>
