...
?>
<center>
<h1>Home  : <span>Birthday Reminder (<font>Upcomming Birthday</font>) </span></h1>
	...
	<tr id="TH">
	 ...
	 <?php 
		...
		//----------------------------------------------------------		
		$ADVISOR_QUERY="SELECT * FROM tbl_advisor where month(advisor_dob) between '$S_M' and '$E_M' AND day(advisor_dob) between '$S_D' and '$E_D' ";
    ...
    <tr id="Today">
            ...
            <td style="text-align:center"><?php echo date('d-M-Y',strtotime($ADVISOR_ROWS['advisor_dob']))?></td>
...
<BR/>
    <table width="99%" id="ExportTable" cellspacing="0" cellpadding="0" border="1" >
      ...
      <?php 
		...
		if(isset($_GET["page"])) { $k=($page-1)*($limit)+1; }	else $k=1;
		//----------------------------------------------------------		
		$CUSTOMER_QUERY="SELECT * FROM tbl_customer where month(customer_dob) between '$S_M' and '$E_M' AND day(customer_dob) between '$S_D' and '$E_D' ";
