...
<center>
<h1><img src="../SpitechImages/AdvisorActivity.png" width="31" height="32" />User  : <span> <?php echo advisor_title?> Daily Activity  </span>
      ...
      <table  width="98%" id="CommonTable" cellspacing="0" class="DontPrint" style="margin-top:0px;">
          ...
          <tr>
            ...
            <td width="9%" >
              <select name="action" id="action" required="required" style="width:100PX;" >
                ...
                <?php $ACT_Q =@mysqli_query($_SESSION['CONN'],"select action_name from  tbl_advisor_action group by action_name order by action_name ");
		...
		{?>
            ...
            <select name="user"  id="user" required="required" style="width:100PX;" >
              ...
              <?php $USER_Q =@mysqli_query($_SESSION['CONN'],"select advisor_code from  tbl_advisor ORDER BY advisor_id");
		while($USER_ROWS=@mysqli_fetch_array($USER_Q))
		{?>
              <option value="<?php echo $USER_ROWS['advisor_code']?>"><?php echo $USER_ROWS['advisor_code']?></option>
              <?php } ?>
            </select></td>
            <td width="4%" >FROM</td>
