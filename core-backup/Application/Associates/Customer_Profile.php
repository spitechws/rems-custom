...
$DBOBJ->ValidateCustomer($_SESSION["advisor_team"],$_GET[md5('customer_id')]);

$advisor_team=$_SESSION["advisor_team"];
?>
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" id="Content">
  <tr>
   ...
   <h2 style="margin:0px; width:502px;">Associate Profile</h2>
   ...
   <tr class="DontPrint">
     ...
     <td colspan="2" id="Value" style="color:red; font-size:13px;">
     <form id="AdvisorForm" name="AdvisorForm" method="get" style="margin:0px; margin-top:3px;">
       <select name="<?php echo md5("customer_id")?>" id="<?php echo md5("customer_id")?>" onchange="AdvisorForm.submit();" style="width:100%; min-width:200px;">
         <option value="">Select A Customer...</option>
         <?php 
			   $CUSTOMER_Q="SELECT customer_id, customer_code, customer_name FROM tbl_customer where customer_id in (select booking_customer_id from tbl_property_booking where booking_advisor_id in($advisor_team) group by  booking_customer_id) ORDER BY customer_name";
    ...
    <tr>
      ...
      <td width="145" rowspan="9" style="vertical-align:top;">
	  <?php $ACTUAL_PHOTO="../SpitechUploads/customer/profile_photo/".$CUSTOMER_ROW['customer_photo'];
		  $exist=file_exists($ACTUAL_PHOTO);
		 
		  if(!isset($_GET[md5('customer_id')]) || $exist!="1" || $CUSTOMER_ROW['customer_photo']=="") { $ACTUAL_PHOTO="../SpitechImages/Customer.png"; }
		 ?>
    ...
    <tr>
      ...
      <td colspan="2" id="Value"><?php ShowDate($CUSTOMER_ROW['customer_anniversary_date']);?></td>
      </tr>
    <tr>
      <td>&nbsp;</td>
  ...
  <tr>
    ...
    <td style="text-align:center"><div align="center" style="width:80px;"><?php echo date('d-M-Y',strtotime($BOOKING_ROWS['booking_date']));?></div></td>
    <td style="text-align:center"><div align="center" style="width:80px;"><?php echo date('d-M-Y',strtotime($BOOKING_ROWS['booking_token_exp_date']));?></div></td>
