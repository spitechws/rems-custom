...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="Content">
  <tr>
    ...
    <th width="5%" rowspan="2">ID&nbsp;CODE</th>
  ...
  <?php
		...
		//----------------------------------------------------------		
		$CUSTOMER_QUERY="select customer_id, customer_title, customer_name, customer_code, customer_mobile, customer_address, customer_city, customer_reg_date,customer_photo from tbl_customer where customer_id in(select booking_customer_id from tbl_property_booking where booking_advisor_id in(".$_SESSION['advisor_team'].") group by booking_customer_id) ";
...
?>
  <tr>
    ...
    <td ><div align="center" style="width:80px;"><?php echo $CUSTOMER_ROWS['customer_code']; ?></div></td>
    ...
    <td >
      <div align="center" style="width:80PX;"><?php ShowDate($CUSTOMER_ROWS['customer_reg_date']); ?>
        </div>
    </td>
    ...
    <td>
	 ...
	 from tbl_property_booking where 
	 booking_customer_id='".$CUSTOMER_ROWS['customer_id']."'
	  and booking_cancel_status=''
	  and approved='1' ";
  ...
  ?> 
      <table width="98%" border="0" cellspacing="1" cellpadding="0" id="SmallTable">
        ...
        <?php while($BOOKING_ROWS=@mysqli_fetch_assoc($BOOKING_Q)) {  ?>
        <tr>
          <td><?php echo ++$i;?></td>
